...
def test_category_detail(auth_client, test_user, goal_category, board_participant):
    url = reverse('goal_category_detail_update_delete', kwargs={'pk': goal_category.id})
    ...
    assert response_data['title'] == goal_category.title
    assert response_data['is_deleted'] == goal_category.is_deleted
...
def test_category_list_limit(auth_client, goal_category_list, board_participant):
    url = reverse('goal_category_list') + '?limit=10'
